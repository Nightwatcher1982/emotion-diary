...
# 上传并执行修复脚本
echo "📤 上传修复脚本到服务器..."
scp -i "$SSH_KEY_PATH" /tmp/simple-fix.sh "$SERVER_USER@$SERVER_HOST:/tmp/"

echo "🔧 执行简化版修复..."
ssh -i "$SSH_KEY_PATH" "$SERVER_USER@$SERVER_HOST" "chmod +x /tmp/simple-fix.sh && /tmp/simple-fix.sh"
